    ...
    try:
        ...
        if len(coins_buffer)==TOTAL_COINS:
            ...
            if bundle_id:
                ...
                image_url = upload_to_cloudflare(filename, f"{bundle_id}.png")
                if image_url:
                    try:
                        supabase.table('bundles').update({"image_url":image_url}).eq("id", bundle_id).execute()
                        logging.info("Updated bundle with image_url.")
                    except Exception as e:
