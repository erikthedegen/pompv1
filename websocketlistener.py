...
IMG_WIDTH = 512
IMG_HEIGHT = 512
BOX_WIDTH = IMG_WIDTH // GRID_COLS   # 256 px
BOX_HEIGHT = IMG_HEIGHT // GRID_ROWS # 128 px

# Font Configuration
FONT_PATH = os.getenv("FONT_PATH", "notosans.ttf")  # Ensure this file is in your project root and accessible
...
def load_font(font_path, size):
    """
    Load a TrueType font. If loading fails, use the default font.
    """
...
def save_bundle_to_db(coins):
    """
    Save a bundle and its coins to the Supabase database.
    ...
    try:
        ...
        logging.error(f"Error saving bundle to database: {e}")
        return None
...
def draw_coin_box(draw, main_image, x, y, coin_data, index):
    ...
    if coin_img_url:
        try:
            ...
            if resp.status_code == 200:
                ...
                logging.info(f"Fetched image for coin {index + 1}: {coin_img_url}")
            else:
    ...
    if desc:
        desc_lines, desc_font = fit_description(
            draw, desc, desc_w, desc_h,
        ...
        if desc_lines is None:
            ...
            try:
                ...
                logging.error(f"Error loading small font: {e}")
        ...
        if "uri" in data and data["uri"]:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    data["metadata_showName"] = metadata.get("showName", "")
                    data["metadata_createdOn"] = metadata.get("createdOn", "")
                    data["metadata_twitter"] = metadata.get("twitter", "")
                    ...
                    logging.warning(f"Failed to fetch metadata for URI {metadata_url}: Status Code {response.status_code}")
                    data["metadata_name"] = ""
                    data["metadata_symbol"] = ""
                    data["metadata_description"] = ""
                    data["metadata_image_official"] = ""
            except Exception as e:
                logging.error(f"Error fetching metadata from {metadata_url}: {e}")
                data["metadata_name"] = ""
                data["metadata_symbol"] = ""
                data["metadata_description"] = ""
                data["metadata_image_official"] = ""
